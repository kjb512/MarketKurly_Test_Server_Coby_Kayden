...
public class CartDao {
    ...
    public List<ProductMiniInfoForCartRes> getUserCartByPackagingType(int userIdx, int packagingTypeIdx) {
        String query = "select *\n" +
                "from (Cart\n" +
        ...
        return this.jdbcTemplate.query(query, (rs, rowNum) -> new ProductMiniInfoForCartRes(rs.getInt("CP.productIdx"),
                ...
                rs.getInt("discount"),
                0,
                rs.getInt("count")), userIdx,packagingTypeIdx);
    }
    ...
    public void addProductCount(int productIdx, int userIdx) {
        String query = "update CartProduct\n" +
                "set count = count + 1\n" +
                "where productIdx = ?\n" +
                "  and (select Cart.cartIdx from Cart where userIdx = ? and Cart.status = 'ACTIVE' ) = cartIdx;";
        this.jdbcTemplate.update(query, productIdx, userIdx);
    }

    // delete 한개 product - status 변경(cartProductIdx 중에 제일 높은)
    public void deleteProductInCart(int productIdx, int userIdx) {
        String query = "update CartProduct\n" +
                "set count = count-1\n" +
                "where (select Cart.cartIdx\n" +
                "       from Cart\n" +
                "       where userIdx = ? and Cart.status = 'ACTIVE') = cartIdx\n" +
                "  and productIdx = ?\n" +
                "  and status = 'ACTIVE'\n" +
                "  and count>0;";
    ...
    public CartUserInfo getCartUserInfo(int userIdx){
        String query = "select *\n" +
                "from (Cart\n" +
        ...
        return this.jdbcTemplate.queryForObject(query,(rs, rowNum) -> new CartUserInfo(rs.getInt("cartIdx"),
                ...
                rs.getString("extraAddress")),userIdx);
    }
